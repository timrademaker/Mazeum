#include "MuseumRoof.h"

#include "../ConstantsFunctionLibrary.h"

AMuseumRoof::AMuseumRoof()
{
    PrimaryActorTick.bCanEverTick = false;
}


void AMuseumRoof::PlaceRoof(const FMapGrid& Grid)
{
	if (RoofMeshComponent)
	{
		...
		// Loop through the binary values to generate the roofs as static mesh components attached to the root of this actor
		for (int32 i = 0; i < gridSize.Y; i++)
		{
			...
			for (int32 j = 0; j < gridSize.X; j++)
			{
				...
				UStaticMeshComponent* meshComponent = NewObject<UStaticMeshComponent>(this, RoofMeshComponent);
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("%s"), TEXT("Unable to create roof as the roof mesh component	is not set!"));
	}
}

void AMuseumRoof::BeginPlay()
{
	Super::BeginPlay();
